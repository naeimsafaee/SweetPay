    ...
    <div class="bread">
        ...
        <div class="info">
            @foreach($types as $type => $name)
                <span>{{__('hotdesk.support_ticket')}} {{$name}} {{Auth::user()->Locale == 'fa' ? fa_number(App\Models\Support::where('is_admin',false)->count()) : App\Models\Support::where('is_admin',false)->count()}}</span>
            @endforeach
        </div>
                ...
                @foreach($dataTypeContent as $data)
                    <tr>
                        ...
                        <td class="no-sort no-click" id="bread-actions">
                            ...
                            <button id="open_chat_naeim" class="openchat1" style="width: 24px;height: 24px;border: 0;background: url(voyager-assets?path=images/icons/note.svg) no-repeat center;cursor: pointer;margin-left: 25px;" data-id="{{$data->id}}"
                                    data-name="{{$data->client->username}}" data-clientid="{{$data->client->id}}"
                                    data-items='@json(\App\Models\Support::query()->where('client_id' , $data->client_id)->orderBy('created_at')->get())'></button>
                            @if($data->has_answer)
